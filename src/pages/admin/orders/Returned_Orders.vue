<template>
  <q-page padding>
    <q-table title="All Returned Orders (Except Pending Status)" :rows="orders" :columns="columns" row-key="id" :loading="loading" >
...
export default {
  setup() {
    ...
    const fetchOrders = async () => {
      loading.value = true;
      try {
        const response = await axios.get('http://localhost/raj-express/backend/controller/adminController/orderController/allOrderControllerReturned.php');
