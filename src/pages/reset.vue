      ...
      <q-page class="flex flex-center">
        <div class="q-pa-md q-ma-md" style="max-width: 500px; width: 100%">
          ...
          <div class="q-card-section">
            <q-form class="q-gutter-md" @submit.prevent="sendGmail">
              <p style="margin-bottom: 0">Enter Email for forgot password!</p>
              <q-input v-model="email" label="Enter Email" type="email" dense outlined style="margin-top: 0" ></q-input>
...
export default {
  ...
  methods:{
    async sendGmail () {
      ...
      try {
        const response = await fetch('http://localhost/raj-express/backend/controller/gmailController/reset_password_request.php', {
          method: 'POST',
          headers: {
